...
void dumpProgramFromEEPROM(int EEPromStart)
{
    ...
    while (true)
    {
        byte = readByteFromEEPROM(EEPromPos++);
...
int findNextStatement(int programPosition)
{
    ...
    while (true)
    {
        char ch = readByteFromEEPROM(programPosition);
...
int findLabelInProgram(char *label, int programPosition)
{
    ...
    while (true)
    {
        ...
        char programByte = readByteFromEEPROM(programPosition++);
        ...
        programByte = readByteFromEEPROM(programPosition++);
...
int getCommandStringLength ( char * pos){
    int result = 0;
    while (*pos != STATEMENT_TERMINATOR & pos != decodeLimit)
    {
        result++;
        pos++;
    }
    return result; // include the null terminator in the length
}

void doRemoteSetNetworkSSID()
{
    int nameLength = getCommandStringLength(decodePos);

    if(nameLength > NETWORK_SETTING_LENGTH){
#ifdef DIAGNOSTICS_ACTIVE
        Serial.println(F("FAIL: network name to large for storage"));
#endif
        return;
    }

    storeBlockIntoEEPROM((uint8_t *) decodePos, nameLength, WIFI_NETWORK_SSID_OFFSET);

    // put the terminator on
    storeByteIntoEEPROM(0, WIFI_NETWORK_SSID_OFFSET+nameLength);

    commitWriteToEEPROM();
}

void doRemoteSetNetworkPassword()
{
    int nameLength = getCommandStringLength(decodePos);

    if(nameLength > NETWORK_SETTING_LENGTH){
#ifdef DIAGNOSTICS_ACTIVE
        Serial.println(F("FAIL: network password to large for storage"));
#endif
        return;
    }

    storeBlockIntoEEPROM((uint8_t *) decodePos, nameLength,WIFI_NETWORK_PASSWORD_OFFSET);

    // put the terminator on
    storeByteIntoEEPROM(0, WIFI_NETWORK_PASSWORD_OFFSET+nameLength);

    commitWriteToEEPROM();
}

void doRemoteSetRobotName()
{
    int nameLength = getCommandStringLength(decodePos);

    if(nameLength > NETWORK_SETTING_LENGTH){
#ifdef DIAGNOSTICS_ACTIVE
        Serial.println(F("FAIL: robot name to large for storage"));
#endif
        return;
    }

    storeBlockIntoEEPROM((uint8_t *) decodePos, nameLength,WIFI_NETWORK_NETWORK_NAME_OFFSET);

    // put the terminator on
    storeByteIntoEEPROM(0, WIFI_NETWORK_NETWORK_NAME_OFFSET+nameLength);

    commitWriteToEEPROM();
}

void remoteNetworkConfig()
{
    if (*decodePos == STATEMENT_TERMINATOR | decodePos == decodeLimit)
    {
#ifdef DIAGNOSTICS_ACTIVE
        Serial.println(F("FAIL: missing network config command"));
#endif
        return;
    }

#ifdef COMMAND_DEBUG
    Serial.println(F(".**netowrk config: "));
#endif
    ...
    case 'S':
    case 's':
        doRemoteSetNetworkSSID();
        break;

    case 'P':
    case 'p':
        doRemoteSetNetworkPassword();
        break;

    case 'N':
    case 'n':
        doRemoteSetRobotName();
        break;
    }
}
...
bool exeuteProgramStatement()
{
    ...
    while (true)
    {
        programByte = readByteFromEEPROM(programCounter++);
